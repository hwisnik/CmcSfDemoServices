...
(function () {
    ...
    function addAuthorization() {
        ...
        var currUrl = "https://erms2dev.cmcenergy.com/CmcSfRestServices/token";
        if (document.URL.indexOf("localhost") !== -1) {
            currUrl = "https://localhost/CmcSfRestServices/token";
        }
        var username = $('#input_username').val();
        var password = $('#input_password').val();
        if (username && username.trim() != "" && password && password.trim() != "") {
            $.ajax({
                url: currUrl, //"https://localhost/CmcSfRestServices/token",
                type: "post",
