rem input parameter from TeamCity Build Step Copy To Folder is the TeamCity working directory/CmcSfDemoServices  (%teamcity.build.workingDir%/CmcSfDemoServices)

rem copy top level (CmcSfDemoServices) config files and web pages
robocopy %1 \\HQ-DEVWEB1\CmcSfDemoServices  /xf "*.cs*" "*.*proj"

rem copy files from bin directory except debug files
robocopy %1/bin \\HQ-DEVWEB1\CmcSfDemoServices\bin /e /xf "*.pdb" 

rem copy EntloggerConfig folder which is at the top level (CmcSfDemoServices) level
robocopy %1/EntLoggerConfig \\HQ-DEVWEB1\CmcSfDemoServices\EntloggerConfig 
